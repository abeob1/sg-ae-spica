                            ...
                            If Functions.CheckHO Then
                                If BusinessObjectInfo.ActionSuccess Then
                                    ...
                                    If oForm.DataSources.DBDataSources.Item(0).GetValue("U_AE_Sync", 0).Trim = "Y" Then
                                        ...
                                        sSQL = "insert into [@AB_BP_SYNC] (Code, Name , U_BPCode, U_Locked,U_Status) values " & _
                                         "((select isnull(max(cast(code as integer)),0) + 1 from [@AB_BP_SYNC]),(select isnull(max(cast(code as integer)),0) + 1 from [@AB_BP_SYNC]),'" & oForm.DataSources.DBDataSources.Item(0).GetValue("CardCode", 0) & "' ,'N', 'Open')"
                                        If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug(" BP SYNC " & sSQL, "BP Add Event")
                                        orset.DoQuery(sSQL)
                            ...
                            If Functions.CheckHO Then
                                If BusinessObjectInfo.ActionSuccess Then
                                    ...
                                    If oForm.DataSources.DBDataSources.Item(0).GetValue("U_AE_Sync", 0).Trim = "Y" Then
                                        ...
                                        sSQL = "insert into [@AB_BP_SYNC] (Code, Name , U_BPCode, U_Locked,U_Status) values " & _
                                         "((select isnull(max(cast(code as integer)),0) + 1 from [@AB_BP_SYNC]),(select isnull(max(cast(code as integer)),0) + 1 from [@AB_BP_SYNC]),'" & oForm.DataSources.DBDataSources.Item(0).GetValue("CardCode", 0) & "' , 'N', 'Open')"
                                        If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug(" BP SYNC " & sSQL, "BP Update Event")
                                        orset.DoQuery(sSQL)
                                    End If
                                End If
                            End If
