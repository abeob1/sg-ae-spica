using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace AE_SPICA_V001
{
    public class Constants
    {
        public const string UserCode = "UserCode";

        public const string UserName = "UserName";

        public const string Password = "Password";

        public const string CompanyCode = "CompanyCode";

        public const string CompanyName = "CompanyName";

        public const string UserRoleName = "UserRoleName";

        public const string SAPDBName = "SAPDBName";

        public const string LoginURL = "Login.aspx";

        public const string IsUpdate = "IsUpdate";

        public const string ScreenName = "ScreenName";

        public const string SessionExpired = "";

        public const string Insert = "INSERT";

        public const string Update = "UPDATE";

        public const string Delete = "DELETE";

        public const string FileRefNo = "FileRefNo";

        public const string ABRFromDate = "ABRFromDate";

        public const string ABRToDate = "ABRToDate";

        public const string DBRFromDate = "DBRFromDate";

        public const string DBRToDate = "DBRToDate";

        public const string DBRFromClaim = "DBRFromClaim";

        public const string DBRToClaim = "DBRToClaim";

        public const string DBRFromComp = "DBRFromComp";

        public const string DBRToComp = "DBRToComp";

        public const string DBRStatus = "DBRStatus";

        public const string NYBRFromDate = "NYBRFromDate";

        public const string NYBRToDate = "NYBRToDate";

        public const string TERFromDate = "TERFromDate";

        public const string TERToDate = "TERToDate";

        public const string NAFRFromDate = "NAFRFromDate";

        public const string NAFRToDate = "NAFRTpDate";

        public const string NAFRFilterBy = "NAFRFilterBy";

        public const string NAFRBranch = "NAFRBranch";

        public const string NAFRClaimHandler = "NAFRClaimHandler";
    }
}