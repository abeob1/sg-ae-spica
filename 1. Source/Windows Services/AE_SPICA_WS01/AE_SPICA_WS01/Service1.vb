Public Class Service1
    ...
    Private Sub BPSync()
        ...
        Try
            ...
            sSQL = "SELECT Top(8)  T0.[Code], T0.[Name], T0.[U_BPCode] FROM [dbo].[@AB_BP_SYNC]  T0 WHERE T0.[U_Locked]  = 'N' and  ISNULL(T0.[U_Status],'') <> 'SUCCESS'"
            ...
            If oDT_BPCode.Rows.Count > 0 Then
                ...
                If oDT_BranchEntities.Rows.Count > 0 Then
                    ...
                    For Each oEntities As DataRow In oDT_BranchEntities.Rows
                        ...
                        For Each oBPCode As DataRow In oDT_BPCode.Rows
                            ...
                            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling Function UpdateBPtoAllBranches()", sFuncName)
                            If UpdateBPtoAllBranches(sBPCode, p_HoCompany, oBranchCompany(imjs), sErrDesc) <> RTN_SUCCESS Then
                                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling Function WriteToLogFile_Sync() - FAIL", sFuncName)
                            ...
                            Else
                                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling Function WriteToLogFile_Sync() - SUCCESS", sFuncName)
                    ...
                    Next
                End If
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Updating the Status to SUCCESS ALL " & sSQL, sFuncName)
                ...
                If oDT_Log.Rows.Count > 0 Then
                    ...
                    Next
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Updating the Status to FAIL " & sSQL, sFuncName)
                    oRset_HL.DoQuery(sSQL)
                End If
            Else
